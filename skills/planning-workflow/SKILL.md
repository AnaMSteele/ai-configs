...
- Every user-facing HTML plan URL must use the full Tailscale MagicDNS name, not a shortname, `localhost`, or `127.0.0.1`. On the default host use `http://mbp.braid-python.ts.net:<port>/...` (canonical plan-reviewer base: `http://mbp.braid-python.ts.net:4317/`). `localhost` / `127.0.0.1` may appear only in private health checks such as `curl -fsS http://127.0.0.1:4317/health`.
