#!/usr/bin/env node
// cdp - lightweight Brave-first Chromium DevTools Protocol CLI
...
// the CDP session open. Brave's "Allow debugging" modal fires once per
...
const INTERNAL_PAGE_SCHEMES = ['chrome://', 'brave://', 'edge://', 'vivaldi://', 'devtools://'];
...
function getWsUrl() {
  ...
  const linuxBrowsers = [
    ...
    'vivaldi', 'vivaldi-snapshot', 'microsoft-edge',
  ];
  ...
  const candidates = [
    ...
    ...(IS_WINDOWS ? ['BraveSoftware/Brave-Browser', 'Google/Chrome', 'Microsoft/Edge'].flatMap(b => {
  ...
  if (!portFile) throw new Error('No DevToolsActivePort found. Enable remote debugging in Brave first, or set CDP_PORT_FILE to another Chromium browser port file.');
...
async function getPages(cdp) {
  ...
  return targetInfos.filter(t => t.type === 'page' && !INTERNAL_PAGE_SCHEMES.some(scheme => t.url.startsWith(scheme)));
}
...
async function runDaemon(targetId) {
  ...
  try {
    ...
    process.stderr.write(`Daemon: cannot connect to Brave/Chromium browser: ${e.message}\n`);
    process.exit(1);
  }
  ...
  // Exit if target goes away or the browser disconnects
...
async function getOrStartTabDaemon(targetId) {
  ...
  // Wait for socket (includes time for user to click Allow in Brave)
  for (let i = 0; i < DAEMON_CONNECT_RETRIES; i++) {
    await sleep(DAEMON_CONNECT_DELAY);
    try { return await connectToSocket(sp); } catch {}
  }
  throw new Error('Daemon failed to start — did you click Allow in Brave or your Chromium browser?');
}
...
const USAGE = `cdp - lightweight Brave-first Chromium DevTools Protocol CLI (no Puppeteer)
...
Usage: cdp <command> [args]
  ...
  open  [url]                       Open a new tab (default: about:blank)
                                    Note: each new tab triggers a fresh "Allow debugging?" prompt in Brave or another Chromium browser
...
async function main() {
  ...
  if (cmd === 'open') {
    ...
    console.log('Note: this tab will need "Allow debugging?" approval on first access in Brave or another Chromium browser.');
    return;
  }
