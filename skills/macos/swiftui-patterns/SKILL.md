...
- Before creating the scaffold, check whether the workspace is already inside a git repo with `git rev-parse --is-inside-work-tree` when repository context matters. Do not initialize a nested repo inside an existing parent checkout.
- For a new app scaffold, create one project-local `script/build_and_run.sh`. If the user explicitly wants Codex app Run-button support too, add `.codex/environments/environment.toml` using the bootstrap contract from `build-run-debug` and its `references/run-button-bootstrap.md` file rather than inventing a second variant here.
...
5. Keep `script/build_and_run.sh` and any optional `.codex/environments/environment.toml` config separate from app source.
