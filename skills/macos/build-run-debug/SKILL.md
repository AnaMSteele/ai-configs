...
optionally wire `.codex/environments/environment.toml` only when the user explicitly
wants Codex app Run-button integration, then use that script as the default
build/run path.
...
1. Discover the project shape.
   - Check whether the workspace is already inside a git repo with `git rev-parse --is-inside-work-tree` when repository context matters.
...
4. If the user explicitly wants Codex app integration, write `.codex/environments/environment.toml` at the project root once the script exists.
   ...
   - Update the existing `Run` action to point at `./script/build_and_run.sh` instead of creating a duplicate action.
   - Keep this optional Codex environment config separate from Swift app source files.
...
- `references/run-button-bootstrap.md`: canonical `build_and_run.sh` shape plus optional Codex `environment.toml` contract.
...
- Do not write `.codex/environments/environment.toml` unless the user explicitly wants Codex integration, and never point the Run action at a stale script path.
...
- the script path and any optional Codex environment action you configured
- the command you ran
