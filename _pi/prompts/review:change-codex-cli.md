...
- Run a single Codex session that:
  ...
  3. adds inline review comments using the `[REVIEW:CODEX] ... [/REVIEW]` format only for blockers, material risks, or missing decisions it finds,
...
- [List only the blocker-level or readiness-changing Codex review findings, or say none]
