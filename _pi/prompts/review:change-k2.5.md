...
## Execution Mode
- This prompt already runs inside the `reviewer-kimi` subagent selected by frontmatter.
- Perform the review directly in this session.
- Do not try to spawn another `reviewer-kimi` task just to satisfy this prompt.
...
```markdown
[REVIEW:Kimi Reviewer] GAP: The plan mentions "user roles" but doesn't define permissions or hierarchy. [/REVIEW]
```
...
- Use `[REVIEW:Kimi Reviewer] ... [/REVIEW]` format.
