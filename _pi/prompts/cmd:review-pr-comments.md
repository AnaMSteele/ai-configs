...
4. **Detect review escapes**:
   - If actionable feedback arrives after this branch already had local Pi quality-reviewer passes, treat it as a `REVIEW_ESCAPE`.
   - A `REVIEW_ESCAPE` means the prior review was not thorough enough. Do not only fix the exact commented line.
   - Add a high-priority task to run an adversarial, scope-bound Pi subagent review cycle after the direct fixes.
5. **Create todos**:
...
3. Run read-only adversarial implementation reviews with Pi `quality-reviewer` and `quality-reviewer-glm` over the current PR diff, the original PR feedback, and the sibling-inspection notes.
   - Do not use `codex-review-partner`, Claude Code, Codex CLI, or any other external reviewer for this escalation.
   - Ask the reviewers to find additional missed issues in the same failure family, not just to validate the direct fix.
