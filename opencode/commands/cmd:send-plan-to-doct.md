...
```bash
bash "$HOME/.config/opencode/skills/doct-document-ops/scripts/publish-coding-plan.sh" --file "$ARGUMENTS"
```

If the plan is not already in a file, write the markdown to a temp file first or pipe it on stdin:

```bash
printf '%s' "$PLAN_MARKDOWN" | bash "$HOME/.config/opencode/skills/doct-document-ops/scripts/publish-coding-plan.sh" --title "Plan Title"
```
