...
print_usage() {
    ...
    echo "  - When using --pi or --all, Pi prompt templates, subagents, and extensions are installed to ~/.pi/agent"
    echo "  - When using --pi or --all, also installs pi extensions via git: pi-dcp, chrome-cdp-skill, pi-rlm"
    echo "  - When using --pi or --all, also installs Pi npm packages via the pi package manager: pi-subagents, @aliou/pi-processes, pi-web-access, pi-mcp-adapter, lsp-pi, @fnnm/pi-ast-grep, pi-updater, pi-interactive-shell, pi-side-chat, pi-powerline-footer, pi-side-agents, pi-multi-pass, pi-no-soft-cursor, @tmustier/pi-files-widget, @tmustier/pi-raw-paste"
