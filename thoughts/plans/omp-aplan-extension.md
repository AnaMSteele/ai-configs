...
- 2026-04-03: Interactive validation showed extension-command `/aplan` still behaved like a normal slash command (`Working`) instead of switching the host into native plan mode. Adjusted the extension so the interactive `input` event rewrites `/aplan ...` into built-in `/plan ...` with the repo-managed OMP workflow bootstrap prompt before slash-command dispatch. This preserves the native instant plan-mode UX while still steering users into the custom repo workflow.
