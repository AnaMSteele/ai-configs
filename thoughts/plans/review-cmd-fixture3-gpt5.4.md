...
[REVIEW:GPT5.4] AMBIGUITY/RISK: "Retries occur up to 3 times" is unclear (3 retries vs 3 total attempts). "Transient 5xx" is underspecified (all 5xx? exclude 501/505? what about 429/408/timeouts/DNS/TLS errors). Specify the exact retryable conditions and backoff/jitter constraints to avoid thundering herds. [/REVIEW]
...
[REVIEW:GPT5.4] AMBIGUITY: Resume Instructions are underspecified. Clarify how to determine "complete" for a phase (must finish Work + Verify), and remind that checkboxes live only in `## Progress` and should be toggled only after Verify passes. [/REVIEW]
