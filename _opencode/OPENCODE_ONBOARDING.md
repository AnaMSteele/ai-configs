...
2. **Use relative paths**: All source paths are relative to the repository root where this onboarding document lives
   - Example: `./_opencode/config-template.json` refers to `config-template.json` in the repository's `_opencode/` directory
...
```bash
# Source: ./_opencode/config-template.json
...
```bash
# Source: ./_opencode/prompts/glm-reasoning.md
# Destination: ~/.config/opencode/prompts/glm-reasoning.md

cp ./_opencode/prompts/glm-reasoning.md ~/.config/opencode/prompts/glm-reasoning.md
```
...
1. **Explore the repository**: Check the `_opencode/agents/` and `_opencode/commands/` directories for customized agents and commands
