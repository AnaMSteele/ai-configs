---
description: Execute a plan with quality-gated phases - each phase gets 1 implementation pass and repeated review/fix passes until no in-scope issues remain and any out-of-scope low-risk items are documented
argument-hint: '<slug | existing-plan-path>'
---

# Run Plan (Quality-Gated Loop)

Execute a plan document phase-by-phase. For each phase, do 1 implementation pass, then repeat review/fix passes until the reviewer finds zero issues or explicitly confirms that no in-scope issues remain and every remaining item is a documented out-of-scope low-risk follow-up. Do not advance with unresolved substantive issues, plan-required work, BDD gaps, verification gaps, or regressions. Capture true out-of-scope follow-up opportunities in a discovery ledger so they can be triaged later without derailing execution.
...
- `plan_slug` = the basename of `plan_path` without its file extension
...
- **Bucket C - Documented out-of-scope discovery**
  - Valuable, but not required to finish the current phase, not required for truthful verification, and not introduced as a regression by this work.
...
Record each documented out-of-scope discovery as:
...
- Why not now: <why it is outside Bucket A/B and not required for truthful verification>
- Recommended follow-up: <next step>
- External scope impact: none | internal only | user-visible | unclear
```

### 2.6) Low-Risk Out-of-Scope Bar

A phase may advance with remaining items only if every item is both low risk and out of scope for the current phase.
...
- It can be safely left to later work without changing the truthfulness of the current phase's tests, verification, or end state.

If any remaining issue fails this bar, it cannot be left for later - keep iterating or ask the user if the disagreement is truly a blocking decision. Do not call in-scope work, BDD gaps, verification gaps, implicit-only coverage, or regressions "low risk" just to advance.

### 3) Execute Phase-by-Phase with Quality Gate

For each phase (tracked by `## Progress`), run 1 implementation pass followed by as many review/fix passes as needed. A phase advances only when a review pass finds zero issues or explicitly confirms that no in-scope issues remain and every remaining item is a documented out-of-scope low-risk follow-up.
...
>
> If you discover worthwhile work that is not required to complete the current phase, not required for truthful verification, and not introduced by this work, classify it using the Discovery Protocol. Record documented out-of-scope discoveries in `<discovery_path>` instead of implementing them now. Only ask the user if the discovery is a true Blocking Decision.
>
...
>
> Fix every issue that is in scope, required for truthful verification, or introduced by this work. Do not just report issues - fix them.
>
> If you find an item that is truly out of scope and low risk under the phase gate rules, record it in `<discovery_path>` as a documented out-of-scope discovery instead of expanding scope. Mention each documented out-of-scope item in your summary.
>
...
> - Use `VERDICT: PASS_LOW_RISK_ONLY` only if no in-scope substantive issues remain and every remaining item is both out of scope and low risk under the phase-gate rules.
...
> - If you used `VERDICT: PASS_LOW_RISK_ONLY`, a `Documented out-of-scope low-risk items:` section listing each item, why it is outside the phase/plan, why it is low risk, and where it is tracked.
...
- `VERDICT: PASS_LOW_RISK_ONLY` -> the phase quality gate is passed only after each out-of-scope low-risk item is recorded in `discovery_path` and referenced in the plan's `## Decisions / Deviations Log`
...
Once the phase is ready to advance (either zero issues found, or only documented out-of-scope low-risk items remain):

1. Flip the phase's checkbox from `- [ ]` to `- [x]` in `## Progress`.
2. If implementation required a decision, revealed a constraint, created a documented out-of-scope discovery, corrected stale verify guidance, required contract/evidence-source drift cleanup, or triggered any test-scope or plan-scope reassessment during implementation or review, ensure `## Decisions / Deviations Log` already contains a structured entry recorded when the event happened; if it does not, append one now, including any test-scope or plan-scope reassessment triggered during implementation or review.
...
- Include an `Unresolved Issues / Documented Out-of-Scope Discoveries` summary in the final readout.
  - If `discovery_path` exists, summarize each open documented out-of-scope discovery and reference `discovery_path`.
  - Summarize any out-of-scope low-risk items that remained at phase completion and reference the plan's `## Decisions / Deviations Log`.
  - If no unresolved issues or documented out-of-scope discoveries remain, state that explicitly.
